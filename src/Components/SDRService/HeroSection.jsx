    ...
    <section className="relative min-h-screen bg-sand overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        {floatingIcons.map((item, index) => (
          ...
          >
            <item.Icon className="w-16 h-16 text-jungle opacity-10" />
          </motion.div>
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 flex flex-col justify-center min-h-screen">
        <motion.div
          ...
          animate={controls}
          className="bg-white dark:bg-jungle-dark backdrop-blur-md rounded-3xl p-8 md:p-12 shadow-2xl"
        >
          <motion.h1
            className="text-4xl md:text-5xl lg:text-6xl font-bold text-jungle dark:text-sand mb-6 leading-tight"
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            {[
              ...
              <div key={index} className="flex flex-col items-center text-center">
                ...
                <span className="text-sm text-jungle dark:text-sand">{item.text}</span>
              </div>
            ))}
          </motion.div>
          ...
          >
            <motion.a
              ...
              target="_blank"
              className="group relative inline-flex items-center justify-center px-8 py-3 overflow-hidden font-medium text-white bg-terra rounded-full shadow-md transition duration-300 ease-out"
              onHoverStart={() => setIsHovered(true)}
              onHoverEnd={() => setIsHovered(false)}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <span className="absolute inset-0 w-full h-full bg-gradient-to-br from-terra to-terra-dark"></span>
              <span className="absolute bottom-0 right-0 block w-64 h-64 mb-32 mr-4 transition duration-500 origin-bottom-left transform rotate-45 translate-x-24 bg-terra-light opacity-30 group-hover:rotate-90 ease"></span>
