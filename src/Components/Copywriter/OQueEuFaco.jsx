      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="flex flex-wrap justify-center items-center">
          {services.map((service, index) => (
            ...
            >
              <div
                className="absolute inset-0 bg-white dark:bg-jungle/90 shadow-lg transition-colors duration-300"
                style={{
                  clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)",
                }}
              >
                <div
                  className="absolute inset-2 bg-gradient-to-br from-terra to-jungle dark:from-terra to-jungle/80 transition-colors duration-300"
                  style={{
                    clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)",
                  }}
                >
                  <div
                    className="absolute inset-1 bg-white dark:bg-jungle/90 flex flex-col items-center justify-center p-4 text-center transition-colors duration-300"
                    style={{
                      clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)",
                    }}
                  >
                    <service.icon className="w-12 h-12 text-terra mb-4" />
                    <h3 className="text-lg font-semibold text-jungle dark:text-sand mb-2 transition-colors duration-300">
                      {service.title}
                    </h3>
                    <p className="text-sm text-gray-700 dark:text-sand/80 transition-colors duration-300">
                      {service.description}
                    </p>
                  </div>
                </div>
        ...
        >
          <a
            href="https://wa.me/71987257532?text=Olá! Gostaria de criar algo incrível para meu negócio."
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block bg-terra hover:bg-terra/90 text-white font-bold py-3 px-8 rounded-full transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1"
          >
            Vamos criar algo incrível juntos?
          </a>
        </motion.div>
